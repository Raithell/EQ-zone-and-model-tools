...
-- Define log_to_file globally (no 'local')
...
local window = assert(iup.dialog{iup.hbox{dir_list, file_list, display; nmargin = "10x10", gap = 10};
    title = "EQG Weapon Model Importer v1.1", menu = menu, size = "650x250"})
...
function window:close_cb()
    if open_dir then
        log_to_file("Main: Closing directory on app exit")
